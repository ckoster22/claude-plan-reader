...
// Prompt Feedback button + overlay (title-bar chrome; never inside #reading-pane).
...
// The backend owns the count; main.ts reads it via a command (never the DOM). The count is made
// VISIBLE via the Prompt Feedback button (applyFeedbackButtonState below).
let commentCount = 0;
...
// ---- feedback-overlay sync hooks (registered by the DOMContentLoaded wiring) ----
...
// ---- Flat sidebar rendering --------------------------------------------------------------
//
// `list_plans` returns records ordered newest-first by mtime. `renderSidebar` walks the list
// top-to-bottom and renders each as a flat row.
...
// Render the flat sidebar from the mtime-ordered records into `listEl`. EXPORTED so the DOM
...
// Re-fetch the list and re-render the sidebar (the mtime sort happens in Rust).
...
export async function openPlan(path: AbsPath, stem: Stem): Promise<void> {
  ...
  if (planListEl) {
    // Iterate every row by data-path so each gets its .active/.unread state updated.
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Prompt Feedback elements (title-bar + overlay; never inside #reading-pane).
  ...
  // ---- Prompt Feedback button + overlay wiring (title-bar domain) ----
