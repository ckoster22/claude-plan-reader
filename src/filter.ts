...
//     keeps its parent — never an orphan) plus the matching subs. Else ⇒ drop the whole group.
  ...
  while (i < records.length) {
    ...
    if (masterMatches) {
      ...
      if (matchedSubs.length > 0) {
        // A sub matched ⇒ keep its master (no orphan) + the matching subs only.
        out.push(rec, ...matchedSubs);
      }
