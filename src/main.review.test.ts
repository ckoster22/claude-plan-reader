...
//     it from pending. Dismiss → decision "allow", removes it.
//   • Fallback: an empty plan_file_path renders the detached planText (degraded) so the review stays
//     actionable.
//
...
function bootDom(): void {
  document.body.innerHTML = `
    ...
    <div class="review-bar hidden" id="review-bar">
      ...
      <button id="review-dismiss"></button>
    ...
    <div class="feedback-overlay hidden" id="feedback-overlay">
      ...
      <button id="feedback-clear"></button>
    </div>
...
// ---------------------------------------------------------------------------------------------
// Submit (deny) / Dismiss (allow) decisions + removal from pending.
// ---------------------------------------------------------------------------------------------
describe("review action bar — Submit (deny) and Dismiss (allow) decisions", () => {
  ...
  it("Dismiss sends decision 'allow' (release to terminal) with the fixed reason and removes it from pending", async () => {
    const path = "/home/u/.claude/plans/Dismiss-Me.md";
    H.rows = [planRow(path, "Dismiss-Me")];
    bootDom();
    await flush();
    await fireReviewRequested("rev-dismiss", path);

    const dismiss = document.querySelector<HTMLButtonElement>("#review-dismiss")!;
    dismiss.click();
    await flush();

    expect(H.responses).toHaveLength(1);
    expect(H.responses[0].reviewId).toBe("rev-dismiss");
    expect(H.responses[0].decision).toBe("allow");
    expect(H.responses[0].reason).toBe("Dismissed in Plan Reader — approve in the terminal.");
    expect(document.querySelector("#review-bar")!.classList.contains("hidden")).toBe(true);
  });
});
...
// ---------------------------------------------------------------------------------------------
// Fallback — empty plan_file_path renders detached text (degraded) but the review stays actionable.
// ---------------------------------------------------------------------------------------------
describe("fallback — empty plan_file_path renders detached plan text (degraded)", () => {
  it("with an empty plan_file_path the handler still renders the plan text and shows the bar in VIEWING mode", async () => {
    bootDom();
    await flush();
    ...
    // Degraded render: the pane shows the "Plan review" header (no real file selected) + the plan text.
    expect(document.querySelector("#doc-filename")!.textContent).toBe("Plan review");
    const pane = document.querySelector<HTMLElement>("#reading-pane")!;
    expect(pane.textContent ?? "").toContain("fallback plan");
    // The bar is still shown (the review is actionable via Dismiss); it derives from pendingReviews.
    expect(document.querySelector("#review-bar")!.classList.contains("hidden")).toBe(false);
